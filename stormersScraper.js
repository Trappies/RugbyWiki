...
const url = `${baseUrl}/stormers/squad`;
...
async function scrapeStormersPlayers() {
  try {
...
scrapeStormersPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'stormers.html';
    ...
    console.log('HTML file updated with the latest Stormers player details.');
  } catch (error) {

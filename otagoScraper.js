...
async function scrapeOtagoPlayers() {
  try {
...
scrapeOtagoPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    ...
    console.log('HTML file updated with the latest Otago player details.');
  } catch (error) {

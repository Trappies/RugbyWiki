...
const url = `${baseUrl}/manawatu/squad`;
...
async function scrapeManawatuPlayers() {
  try {
...
scrapeManawatuPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'manawatu.html';
    ...
    console.log('HTML file updated with the latest Manawatu player details.');
  } catch (error) {

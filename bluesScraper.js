...
const url = `${baseUrl}/auckland-blues/squad`;
...
async function scrapeBluesPlayers() {
  try {
...
scrapeBluesPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    ...
    console.log('HTML file updated with the latest Blues player details.');
  } catch (error) {

...
const url = `${baseUrl}/leicester-tigers/squad`;
...
async function scrapeLeicesterTigersPlayers() {
  try {
...
scrapeLeicesterTigersPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'leicester_tigers.html';
    ...
    console.log('HTML file updated with the latest Leicester Tigers player details.');
  } catch (error) {

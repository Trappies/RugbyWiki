...
const url = `${baseUrl}/melbourne-rebels/squad`;
...
async function scrapeRebelsPlayers() {
  try {
...
scrapeRebelsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'rebels.html';
    ...
    console.log('HTML file updated with the latest Rebels player details.');
  } catch (error) {

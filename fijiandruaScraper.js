...
const url = `${baseUrl}/fiji-warriors/squad`;
...
async function scrapeFijianDruaPlayers() {
  try {
...
scrapeFijianDruaPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'fijian_drua.html';
    ...
    console.log('HTML file updated with the latest Fijian Drua player details.');
  } catch (error) {

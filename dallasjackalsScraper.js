...
const url = `${baseUrl}/dallas-jackals/squad`;
...
async function scrapeDallasPlayers() {
  try {
...
scrapeDallasPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'dallas_jackals.html';
    ...
    console.log('HTML file updated with the latest Dallas Jackals player details.');
  } catch (error) {

...
const url = `${baseUrl}/tasman-makos/squad`;
...
async function scrapeTasmanMakosPlayers() {
  try {
...
scrapeTasmanMakosPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'tasman_makos.html';
    ...
    console.log('HTML file updated with the latest Tasman Makos player details.');
  } catch (error) {

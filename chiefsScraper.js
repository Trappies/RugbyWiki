...
const url = `${baseUrl}/chiefs/squad`;
...
async function scrapeChiefsPlayers() {
  try {
...
scrapeChiefsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'chiefs.html';
    ...
    console.log('HTML file updated with the latest Chiefs player details.');
  } catch (error) {

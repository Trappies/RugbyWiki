...
async function scrapeLyonPlayers() {
  try {
...
scrapeLyonPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    ...
    console.log('HTML file updated with the latest Lyon player details.');
  } catch (error) {

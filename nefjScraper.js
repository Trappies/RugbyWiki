...
const url = `${baseUrl}/new-england-free-jacks/squad`;
...
async function scrapeFreeJacksPlayers() {
  try {
...
scrapeFreeJacksPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'new_england_free_jacks.html';
    ...
    console.log('HTML file updated with the latest New England Free Jacks player details.');
  } catch (error) {

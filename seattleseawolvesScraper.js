...
const url = `${baseUrl}/seattle-seawolves/squad`;
...
async function scrapeSeattleSeawolvesPlayers() {
  try {
...
scrapeSeattleSeawolvesPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'seattle_seawolves.html';
    ...
    console.log('HTML file updated with the latest Seattle Seawolves player details.');
  } catch (error) {

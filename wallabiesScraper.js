...
const url = `${baseUrl}/australia/squad`;
...
async function scrapeWallabiesPlayers() {
  try {
...
scrapeWallabiesPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'wallabies.html';
    ...
    console.log('HTML file updated with the latest Wallabies player details.');
  } catch (error) {

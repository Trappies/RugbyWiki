...
async function scrapeEPCRFixtures() {
    try {
        const response = await axios.get('https://www.rugbypass.com/challenge-cup/fixtures-results/');
        ...
        console.error('Error scraping EPCR fixtures:', error.message);
        return [];
    }
}
...
async function updateHTMLWithFixtures(fixtures) {
    try {
        ...
        console.log('HTML file updated with the latest EPCR fixtures.');
    } catch (error) {
        console.error('Error updating HTML file:', error.message);
    }
}

scrapeEPCRFixtures().then((fixtures) => {
    updateHTMLWithFixtures(fixtures);
  });

async function scrapeEPCRStandings() {
    try {
        const response = await axios.get('https://www.rugbypass.com/challenge-cup/standings/');
        const $ = cheerio.load(response.data);
...
async function updateHTMLWithStandings(standings) {
    try {
        ...
        const standingsContainer = $('#content2');
        ...
        const headerRow = `
            <tr class='head'>
                ...
                <th scope="col">PA</th>
                <th scope="col">PD</th>
                <th scope="col">BP T</th>
                <th scope="col">BP-7</th>
                <th scope="col">BP</th>
        ...
        // Split the standings into 4 tables
        const standingsChunks = chunkArray(standings, 6);

        standingsChunks.forEach((tableStandings, tableIndex) => {
            const tableId = `standingsTable${tableIndex + 1}Body`;

            // Create a new container div for each table
            const tableContainer = $('<div class="standings-table-container"></div>');

            // Create a new table for each chunk
            const table = $('<table class="table"></table>');
            const tableBody = $(`<tbody id="${tableId}"></tbody>`);

            tableStandings.forEach((team) => {
                const tableRow = `
                    <tr>
            ...
            // Append the header row and the new table to the container
            table.append(headerRow);
            table.append(tableBody);
            tableContainer.append(table);
            
            // Append the container to the standings container
            standingsContainer.append(tableContainer);
        });

        fs.writeFileSync(htmlFilePath, $.html());
        console.log('HTML file updated with the latest EPCR standings.');
    } catch (error) {
...
scrapeEPCRStandings().then((standings) => {

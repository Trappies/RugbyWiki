...
const url = `${baseUrl}/northland/squad`;
...
async function scrapeNorthlandPlayers() {
  try {
...
scrapeNorthlandPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'northland.html';
    ...
    console.log('HTML file updated with the latest Northland player details.');
  } catch (error) {

...
const url = `${baseUrl}/union-bordeaux-begles/squad`;
...
async function scrapeBordeauxPlayers() {
  try {
...
scrapeBordeauxPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'bordeaux.html';
    ...
    console.log('HTML file updated with the latest Bordeaux player details.');
  } catch (error) {

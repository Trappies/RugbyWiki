...
const url = `${baseUrl}/nola-gold/squad`;
...
async function scrapeNolaGoldPlayers() {
  try {
...
scrapeNolaGoldPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'nola_gold.html';
    ...
    console.log('HTML file updated with the latest Nola Gold player details.');
  } catch (error) {

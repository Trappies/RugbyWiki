...
const url = `${baseUrl}/glasgow-warriors/squad`;
...
async function scrapeGlasgowWarriorsPlayers() {
  try {
...
scrapeGlasgowWarriorsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'glasgow_warriors.html';
    ...
    console.log('HTML file updated with the latest Glasgow Warriors player details.');
  } catch (error) {

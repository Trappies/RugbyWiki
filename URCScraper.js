...
async function scrapeURCFixtures() {
    try {
        const response = await axios.get('https://www.rugbypass.com/united-rugby-championship/fixtures-results/');
...
async function updateHTMLWithFixtures(fixtures) {
    try {
        ...
        console.log('HTML file updated with the latest URC fixtures.');
    } catch (error) {
        console.error('Error updating HTML file:', error.message);
    }
}

scrapeURCFixtures().then((fixtures) => {
    updateHTMLWithFixtures(fixtures);
  });

async function scrapeURCStandings() {
    try {
        const response = await axios.get('https://www.rugbypass.com/united-rugby-championship/standings/');
        const $ = cheerio.load(response.data);
...
async function updateHTMLWithStandings(standings) {
    try {
        ...
        fs.writeFileSync(htmlFilePath, $.html());
        console.log('HTML file updated with the latest URC standings.');
    } catch (error) {
...
scrapeURCStandings().then((standings) => {

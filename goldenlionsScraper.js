...
const url = `${baseUrl}/golden-lions/squad`;
...
async function scrapeGoldenLionsPlayers() {
  try {
...
scrapeGoldenLionsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'golden_lions.html';
    ...
    console.log('HTML file updated with the latest Golden Lions player details.');
  } catch (error) {

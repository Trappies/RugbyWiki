...
const url = `${baseUrl}/connacht-rugby/squad`;
...
async function scrapeConnachtPlayers() {
  try {
...
scrapeConnachtPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'connacht.html';
    ...
    console.log('HTML file updated with the latest Connacht player details.');
  } catch (error) {

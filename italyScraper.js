...
async function scrapeItalyPlayers() {
  try {
...
scrapeItalyPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    ...
    console.log('HTML file updated with the latest Italy player details.');
  } catch (error) {

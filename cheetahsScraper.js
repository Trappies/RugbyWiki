...
const url = `${baseUrl}/app/public/index.php/cheetahs/squad`;
...
async function scrapeCheetahsPlayers() {
  try {
...
scrapeCheetahsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'cheetahs.html';
    ...
    console.log('HTML file updated with the latest Cheetahs player details.');
  } catch (error) {

...
const url = `${baseUrl}/munster-rugby/squad`;
...
async function scrapeMunsterPlayers() {
  try {
...
scrapeMunsterPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'munster.html';
    ...
    console.log('HTML file updated with the latest Munsrer player details.');
  } catch (error) {

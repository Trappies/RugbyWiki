...
const url = `${baseUrl}/queensland-reds/squad`;
...
async function scrapeRedsPlayers() {
  try {
...
scrapeRedsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    ...
    console.log('HTML file updated with the latest Reds player details.');
  } catch (error) {

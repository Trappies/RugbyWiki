...
const url = `${baseUrl}/app/public/index.php/griffons/squad`;
...
async function scrapeGriffonsPlayers() {
  try {
...
scrapeGriffonsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'griffons.html';
    ...
    console.log('HTML file updated with the latest Griffons player details.');
  } catch (error) {

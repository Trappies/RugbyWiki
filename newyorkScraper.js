...
const url = `${baseUrl}/rugby-united-new-york/squad`;
...
async function scrapeNewYorkPlayers() {
  try {
...
scrapeNewYorkPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'rugby_new_york.html';
    ...
    console.log('HTML file updated with the latest Rugby New York player details.');
  } catch (error) {

...
const url = `${baseUrl}/ireland/squad`;
...
async function scrapeIrelandPlayers() {
  try {
...
scrapeIrelandPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'ireland.html';
    ...
    console.log('HTML file updated with the latest France player details.');
  } catch (error) {

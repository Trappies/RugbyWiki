...
async function scrapeMiamiPlayers() {
  try {
...
scrapeMiamiPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'miami_sharks.html';
    ...
    console.log('HTML file updated with the latest Miami Sharks player details.');
  } catch (error) {

...
const url = `${baseUrl}/harlequins/squad`;
...
async function scrapeHarlequinsPlayers() {
  try {
...
scrapeHarlequinsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'harlequins.html';
    ...
    console.log('HTML file updated with the latest Harlequins player details.');
  } catch (error) {

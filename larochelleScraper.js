...
const url = `${baseUrl}/la-rochelle/squad`;
...
async function scrapeLaRochellePlayers() {
  try {
...
scrapeLaRochellePlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'la_rochelle.html';
    ...
    console.log('HTML file updated with the latest La Rochelle player details.');
  } catch (error) {

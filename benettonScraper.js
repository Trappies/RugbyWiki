...
const url = `${baseUrl}/benetton-treviso/squad`;
...
async function scrapeBenettonPlayers() {
  try {
...
scrapeBenettonPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'benetton.html';
    ...
    console.log('HTML file updated with the latest Benetton player details.');
  } catch (error) {

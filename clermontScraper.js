...
const url = `${baseUrl}/clermont/squad`;
...
async function scrapeClermontPlayers() {
  try {
...
scrapeClermontPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'clermont.html';
    ...
    console.log('HTML file updated with the latest Clermont player details.');
  } catch (error) {

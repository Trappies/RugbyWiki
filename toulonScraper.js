...
const url = `${baseUrl}/app/public/index.php/rc-toulon/squad`;
...
async function scrapeToulonPlayers() {
  try {
...
scrapeToulonPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'toulon.html';
    ...
    console.log('HTML file updated with the latest Toulon player details.');
  } catch (error) {

...
const url = `${baseUrl}/south-africa/squad`;
...
async function scrapeSpringboksPlayers() {
  try {
...
scrapeSpringboksPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'springboks.html';
    ...
    console.log('HTML file updated with the latest Springboks player details.');
  } catch (error) {

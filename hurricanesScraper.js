...
const url = `${baseUrl}/hurricanes/squad`;
...
async function scrapeHurricanesPlayers() {
  try {
...
scrapeHurricanesPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'hurricanes.html';
    ...
    console.log('HTML file updated with the latest Hurricanes player details.');
  } catch (error) {

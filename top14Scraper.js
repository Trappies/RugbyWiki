...
async function scrapeTop14Fixtures() {
    try {
        const response = await axios.get('https://www.rugbypass.com/top-14/fixtures-results/');
...
scrapeTop14Fixtures().then((fixtures) => {
    updateHTMLWithFixtures(fixtures);
  });

async function scrapeTop14Standings() {
    try {
        const response = await axios.get('https://www.rugbypass.com/top-14/standings/');
        const $ = cheerio.load(response.data);
...
scrapeTop14Standings().then((standings) => {

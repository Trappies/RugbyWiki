...
const url = `${baseUrl}/crusaders/squad`;
...
async function scrapeCrusadersPlayers() {
  try {
...
scrapeCrusadersPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'crusaders.html';
    ...
    console.log('HTML file updated with the latest Crusaders player details.');
  } catch (error) {

...
const url = `${baseUrl}/waikato/squad`;
...
async function scrapeWaikatoPlayers() {
  try {
...
scrapeWaikatoPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'waikato.html';
    ...
    console.log('HTML file updated with the latest Waikato player details.');
  } catch (error) {

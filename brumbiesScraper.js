...
const url = `${baseUrl}/brumbies/squad`;
...
async function scrapeBrumbiesPlayers() {
  try {
...
scrapeBrumbiesPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'brumbies.html';
    ...
    console.log('HTML file updated with the latest Brumbies player details.');
  } catch (error) {

...
const url = `${baseUrl}/team/squad/20888`;
...
async function scrapeMoanaPasifikaPlayers() {
  try {
...
scrapeMoanaPasifikaPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'moana_pasifika.html';
    ...
    console.log('HTML file updated with the latest Moana Pasifika player details.');
  } catch (error) {

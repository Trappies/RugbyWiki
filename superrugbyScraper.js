...
async function scrapeSixNationsFixtures() {
    try {
        const response = await axios.get('https://www.rugbypass.com/super-rugby/fixtures-results/');
...
async function updateHTMLWithFixtures(fixtures) {
    try {
        const htmlFilePath = 'superrugby.html';
...
async function scrapeSixNationsStandings() {
    try {
        const response = await axios.get('https://www.rugbypass.com/super-rugby/standings/');
        const $ = cheerio.load(response.data);

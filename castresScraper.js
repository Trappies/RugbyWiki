...
const url = `${baseUrl}/castres-olympique/squad`;
...
async function scrapeCastresPlayers() {
  try {
...
scrapeCastresPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'castres.html';
    ...
    console.log('HTML file updated with the latest Castres player details.');
  } catch (error) {

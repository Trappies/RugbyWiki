...
const url = `${baseUrl}/rugby-atl/squad`;
...
async function scrapeRugbyATLPlayers() {
  try {
...
scrapeRugbyATLPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'rugby_atl.html';
    ...
    console.log('HTML file updated with the latest Rugby ATL player details.');
  } catch (error) {

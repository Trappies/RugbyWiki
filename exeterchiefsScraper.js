...
const url = `${baseUrl}/exeter-chiefs/squad`;
...
async function scrapeExeterChiefsPlayers() {
  try {
...
scrapeExeterChiefsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'exeter_chiefs.html';
    ...
    console.log('HTML file updated with the latest Exeter Chiefs player details.');
  } catch (error) {

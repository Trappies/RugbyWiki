...
const url = `${baseUrl}/utah-warriors/squad`;
...
async function scrapeUtahWarriorsPlayers() {
  try {
...
scrapeUtahWarriorsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'utah_warriors.html';
    ...
    console.log('HTML file updated with the latest Utah Warriors player details.');
  } catch (error) {

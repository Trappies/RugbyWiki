...
const url = `${baseUrl}/toronto-arrows/squad`;
...
async function scrapeTorontoArrowsPlayers() {
  try {
...
scrapeTorontoArrowsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'toronto_arrows.html';
    ...
    console.log('HTML file updated with the latest Toronto Arrows player details.');
  } catch (error) {

...
async function scrapeBullsPlayers() {
  try {
...
scrapeBullsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    ...
    console.log('HTML file updated with the latest Bulls player details.');
  } catch (error) {

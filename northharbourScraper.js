...
const url = `${baseUrl}/north-harbour/squad`;
...
async function scrapeNorthHarbourPlayers() {
  try {
...
scrapeNorthHarbourPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'north_harbour.html';
    ...
    console.log('HTML file updated with the latest North Harbour player details.');
  } catch (error) {

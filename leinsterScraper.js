...
const url = `${baseUrl}/leinster-rugby/squad`;
...
async function scrapeLeinsterPlayers() {
  try {
...
scrapeLeinsterPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'leinster.html';
    ...
    console.log('HTML file updated with the latest Leinster player details.');
  } catch (error) {

...
const url = `${baseUrl}/england/squad`;
...
async function scrapeEnglandPlayers() {
  try {
...
scrapeEnglandPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'england.html';
    ...
    console.log('HTML file updated with the latest England player details.');
  } catch (error) {

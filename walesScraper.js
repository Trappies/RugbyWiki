...
async function scrapeWalesPlayers() {
  try {
...
scrapeWalesPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    ...
    console.log('HTML file updated with the latest Wales player details.');
  } catch (error) {

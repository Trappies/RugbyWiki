...
const url = `${baseUrl}/zebre-parma/squad`;
...
async function scrapeZebreParmaPlayers() {
  try {
...
scrapeZebreParmaPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'zebre_parma.html';
    ...
    console.log('HTML file updated with the latest Zebra Parma player details.');
  } catch (error) {

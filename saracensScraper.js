...
const url = `${baseUrl}/saracens/squad`;
...
async function scrapeSaracensPlayers() {
  try {
...
scrapeSaracensPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'saracens.html';
    ...
    console.log('HTML file updated with the latest Saracens player details.');
  } catch (error) {

...
const url = `${baseUrl}/old-glory-dc/squad`;
...
async function scrapeOldGloryPlayers() {
  try {
...
scrapeOldGloryPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'old_glory_dc.html';
    ...
    console.log('HTML file updated with the latest Old Glory DC player details.');
  } catch (error) {

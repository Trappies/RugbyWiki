...
const url = `${baseUrl}/oyonnax/squad`;
...
async function scrapeOyonnaxPlayers() {
  try {
...
scrapeOyonnaxPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'oyonnax.html';
    ...
    console.log('HTML file updated with the latest Oyonnax player details.');
  } catch (error) {

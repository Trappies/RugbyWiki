...
const url = `${baseUrl}/hawkes-bay/squad`;
...
async function scrapeHawkesBayPlayers() {
  try {
...
scrapeHawkesBayPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'hawkes_bay.html';
    ...
    console.log('HTML file updated with the latest Hawkes Bay player details.');
  } catch (error) {

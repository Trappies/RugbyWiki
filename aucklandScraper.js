...
const url = `${baseUrl}/auckland/squad`;
...
async function scrapeAuckalndPlayers() {
  try {
...
scrapeAuckalndPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'auckland.html';
    ...
    console.log('HTML file updated with the latest Auckland player details.');
  } catch (error) {

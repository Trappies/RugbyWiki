...
const url = `${baseUrl}/counties-manukau/squad`;
...
async function scrapeManukauPlayers() {
  try {
...
scrapeManukauPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'counties_manukau.html';
    ...
    console.log('HTML file updated with the latest Counties Manukau player details.');
  } catch (error) {

...
const url = `${baseUrl}/newcastle-falcons/squad`;
...
async function scrapeNewcastleFalconsPlayers() {
  try {
...
scrapeNewcastleFalconsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'newcastle_falcons.html';
    ...
    console.log('HTML file updated with the latest Newcastle Falcons player details.');
  } catch (error) {

...
const url = `${baseUrl}/app/public/index.php/western-province/squad`;
...
async function scrapeWesternProvincePlayers() {
  try {
...
scrapeWesternProvincePlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'western_province.html';
    ...
    console.log('HTML file updated with the latest Western Province player details.');
  } catch (error) {

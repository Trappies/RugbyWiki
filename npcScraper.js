...
async function scrapeNPCFixtures() {
    try {
        const response = await axios.get('https://www.rugbypass.com/bunnings-npc/fixtures-results/');
...
scrapeNPCFixtures().then((fixtures) => {
    updateHTMLWithFixtures(fixtures);
  });

async function scrapeNPCStandings() {
    try {
        const response = await axios.get('https://www.rugbypass.com/bunnings-npc/standings/');
        const $ = cheerio.load(response.data);
...
async function updateHTMLWithStandings(standings) {
    try {
        ...
        fs.writeFileSync(htmlFilePath, $.html());
        console.log('HTML file updated with the latest NPC standings.');
    } catch (error) {
...
scrapeNPCStandings().then((standings) => {

...
const url = `${baseUrl}/edinburgh-rugby/squad`;
...
async function scrapeEdinburghPlayers() {
  try {
...
scrapeEdinburghPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'edinburgh.html';
    ...
    console.log('HTML file updated with the latest Edinburgh player details.');
  } catch (error) {

...
const url = `${baseUrl}/wellington/squad`;
...
async function scrapeWellingtonPlayers() {
  try {
...
scrapeWellingtonPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'wellington.html';
    ...
    console.log('HTML file updated with the latest Wellington player details.');
  } catch (error) {

...
const url = `${baseUrl}/canterbury/squad`;
...
async function scrapeCanterburyPlayers() {
  try {
...
scrapeCanterburyPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'canterbury.html';
    ...
    console.log('HTML file updated with the latest Canterbury player details.');
  } catch (error) {

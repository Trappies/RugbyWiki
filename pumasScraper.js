...
const url = `${baseUrl}/app/public/index.php/steval-pumas/squad`;
...
async function scrapePumasPlayers() {
  try {
...
scrapePumasPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    ...
    console.log('HTML file updated with the latest Pumas player details.');
  } catch (error) {

...
const url = `${baseUrl}/app/public/index.php/dragons/squad`;
...
async function scrapeDragonsPlayers() {
  try {
...
scrapeDragonsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'dragons.html';
    ...
    console.log('HTML file updated with the latest Dragons player details.');
  } catch (error) {

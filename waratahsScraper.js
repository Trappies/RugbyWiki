...
const url = `${baseUrl}/nsw-waratahs/squad`;
...
async function scrapeWaratahsPlayers() {
  try {
...
scrapeWaratahsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'waratahs.html';
    ...
    console.log('HTML file updated with the latest Waratahs player details.');
  } catch (error) {

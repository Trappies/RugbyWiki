...
const url = `${baseUrl}/app/public/index.php/san-diego-legion/squad`;
...
async function scrapeLegionPlayers() {
  try {
...
scrapeLegionPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'san_diego_legion.html';
    ...
    console.log('HTML file updated with the latest San Diego Legion player details.');
  } catch (error) {

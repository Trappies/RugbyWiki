...
const url = `${baseUrl}/ulster-rugby/squad`;
...
async function scrapeUlsterPlayers() {
  try {
...
scrapeUlsterPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'ulster.html';
    ...
    console.log('HTML file updated with the latest Ulster player details.');
  } catch (error) {

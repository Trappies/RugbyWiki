...
const url = `${baseUrl}/app/public/index.php/bay-of-plenty/squad`;
...
async function scrapeBayOfPlentyPlayers() {
  try {
...
scrapeBayOfPlentyPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'bay_of_plenty.html';
    ...
    console.log('HTML file updated with the latest Bay of Plenty player details.');
  } catch (error) {

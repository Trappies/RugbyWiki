...
const url = `${baseUrl}/griquas/squad`;
...
async function scrapeGriquasPlayers() {
  try {
...
scrapeGriquasPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'griquas.html';
    ...
    console.log('HTML file updated with the latest Griquas player details.');
  } catch (error) {

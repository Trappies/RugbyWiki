...
const url = `${baseUrl}/app/public/index.php/stade-francais/squad`;
...
async function scrapeStadeFrancaisPlayers() {
  try {
...
scrapeStadeFrancaisPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'stade_francais.html';
    ...
    console.log('HTML file updated with the latest Stade Francais player details.');
  } catch (error) {

...
const url = `${baseUrl}/cardiff-blues/squad`;
...
async function scrapeCardiffPlayers() {
  try {
...
scrapeCardiffPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'cardiff.html';
    ...
    console.log('HTML file updated with the latest Cardiff player details.');
  } catch (error) {

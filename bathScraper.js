...
const url = `${baseUrl}/bath-rugby/squad`;
...
async function scrapeBathPlayers() {
  try {
...
scrapeBathPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    ...
    console.log('HTML file updated with the latest Bath player details.');
  } catch (error) {

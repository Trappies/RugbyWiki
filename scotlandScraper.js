...
const url = `${baseUrl}/scotland/squad`;
...
async function scrapeScotlandPlayers() {
  try {
...
scrapeScotlandPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'scotland.html';
    ...
    console.log('HTML file updated with the latest Scotland player details.');
  } catch (error) {

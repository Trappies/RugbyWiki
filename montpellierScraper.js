...
const url = `${baseUrl}/montpellier/squad`;
...
async function scrapeMontpellierPlayers() {
  try {
...
scrapeMontpellierPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'montpellier.html';
    ...
    console.log('HTML file updated with the latest Montpellier player details.');
  } catch (error) {

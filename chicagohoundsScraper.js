...
const url = `${baseUrl}/team/squad/20919`;
...
async function scrapeChicagoPlayers() {
  try {
...
scrapeChicagoPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'chicago_hounds.html';
    ...
    console.log('HTML file updated with the latest Chicago Hounds player details.');
  } catch (error) {

...
const url = `${baseUrl}/bristol/squad`;
...
async function scrapeBristolBearsPlayers() {
  try {
...
scrapeBristolBearsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'bristol_bears.html';
    ...
    console.log('HTML file updated with the latest Bristol Bears player details.');
  } catch (error) {

...
const url = `${baseUrl}/highlanders/squad`;
...
async function scrapeHighlandersPlayers() {
  try {
...
scrapeHighlandersPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'highlanders.html';
    ...
    console.log('HTML file updated with the latest Highlanders player details.');
  } catch (error) {

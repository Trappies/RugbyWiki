...
const url = `${baseUrl}/app/public/index.php/scarlets/squad`;
...
async function scrapeScarletsPlayers() {
  try {
...
scrapeScarletsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'scarlets.html';
    ...
    console.log('HTML file updated with the latest Scarlets player details.');
  } catch (error) {

...
const url = `${baseUrl}/racing-92/squad`;
...
async function scrapeRacing92Players() {
  try {
...
scrapeRacing92Players().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'racing_92.html';
    ...
    console.log('HTML file updated with the latest Racing 92 player details.');
  } catch (error) {

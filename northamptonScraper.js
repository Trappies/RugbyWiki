...
const url = `${baseUrl}/northampton-saints/squad`;
...
async function scrapeNorthamptonSaintsPlayers() {
  try {
...
scrapeNorthamptonSaintsPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'northampton_saints.html';
    ...
    console.log('HTML file updated with the latest Northampton Saints player details.');
  } catch (error) {

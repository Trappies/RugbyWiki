...
const url = `${baseUrl}/app/public/index.php/bayonne/squad`;
...
async function scrapeBayonnePlayers() {
  try {
...
scrapeBayonnePlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'bayonne.html';
    ...
    console.log('HTML file updated with the latest Bayonne player details.');
  } catch (error) {

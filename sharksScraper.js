...
const url = `${baseUrl}/sharks/squad`;
...
async function scrapeSharksPlayers() {
  try {
...
scrapeSharksPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'sharks.html';
    ...
    console.log('HTML file updated with the latest Sharks player details.');
  } catch (error) {

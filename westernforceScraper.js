...
const url = `${baseUrl}/western-force/squad`;
...
async function scrapeWesternForcePlayers() {
  try {
...
scrapeWesternForcePlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'western_force.html';
    ...
    console.log('HTML file updated with the latest Western Force player details.');
  } catch (error) {

...
const url = `${baseUrl}/houston-sabercats/squad`;
...
async function scrapeHoustonPlayers() {
  try {
...
scrapeHoustonPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'houston_sabercats.html';
    ...
    console.log('HTML file updated with the latest Houston Sabercats player details.');
  } catch (error) {

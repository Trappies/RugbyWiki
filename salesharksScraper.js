...
const url = `${baseUrl}/app/public/index.php/sale-sharks/squad`;
...
async function scrapeSaleSharksPlayers() {
  try {
...
scrapeSaleSharksPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'sale_sharks.html';
    ...
    console.log('HTML file updated with the latest Sale Sharks player details.');
  } catch (error) {

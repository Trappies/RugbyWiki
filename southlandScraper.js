...
const url = `${baseUrl}/southland/squad`;
...
async function scrapeSouthlandPlayers() {
  try {
...
scrapeSouthlandPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'southland_stags.html';
    ...
    console.log('HTML file updated with the latest Southland Stags player details.');
  } catch (error) {

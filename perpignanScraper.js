...
const url = `${baseUrl}/app/public/index.php/perpignan/squad`;
...
async function scrapePerpignanPlayers() {
  try {
...
scrapePerpignanPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'perpignan.html';
    ...
    console.log('HTML file updated with the latest Perpignan player details.');
  } catch (error) {

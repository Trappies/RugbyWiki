...
const url = `${baseUrl}/argentina/squad`;
...
async function scrapeLosPumasPlayers() {
  try {
...
scrapeLosPumasPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'los_pumas.html';
    ...
    console.log('HTML file updated with the latest Los Pumas player details.');
  } catch (error) {

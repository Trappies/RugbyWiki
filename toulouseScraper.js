...
const url = `${baseUrl}/toulouse/squad`;
...
async function scrapeToulousePlayers() {
  try {
...
scrapeToulousePlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'toulouse.html';
    ...
    console.log('HTML file updated with the latest Toulouse player details.');
  } catch (error) {

...
async function scrapeMLRFixtures() {
    try {
        const response = await axios.get('https://www.rugbypass.com/major-league-rugby/fixtures-results/');
...
async function updateHTMLWithFixtures(fixtures) {
    try {
        ...
        console.log('HTML file updated with the latest MLR fixtures.');
    } catch (error) {
        console.error('Error updating HTML file:', error.message);
    }
}

scrapeMLRFixtures().then((fixtures) => {
    updateHTMLWithFixtures(fixtures);
  });

async function scrapeMLRStandings() {
    try {
        const response = await axios.get('https://www.rugbypass.com/major-league-rugby/standings/');
        const $ = cheerio.load(response.data);
...
async function updateHTMLWithStandings(standings) {
    try {
        ...
        fs.writeFileSync(htmlFilePath, $.html());
        console.log('HTML file updated with the latest MLR standings.');
    } catch (error) {
...
scrapeMLRStandings().then((standings) => {

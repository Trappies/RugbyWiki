...
const url = `${baseUrl}/gloucester/squad`;
...
async function scrapeGloucesterPlayers() {
  try {
...
scrapeGloucesterPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'gloucester.html';
    ...
    console.log('HTML file updated with the latest Gloucester player details.');
  } catch (error) {

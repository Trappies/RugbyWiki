...
const url = `${baseUrl}/new-zealand/squad`;
...
async function scrapeAllBlacksPlayers() {
  try {
...
scrapeAllBlacksPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'all_blacks.html';
    ...
    console.log('HTML file updated with the latest All Blacks player details.');
  } catch (error) {

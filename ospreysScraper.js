...
const url = `${baseUrl}/ospreys/squad`;
...
async function scrapeOspreysPlayers() {
  try {
...
scrapeOspreysPlayers().then((players) => {
  if (players.length > 0) {
...
async function updateHTMLWithPlayers(playerData) {
  try {
    const htmlFilePath = 'ospreys.html';
    ...
    console.log('HTML file updated with the latest Ospreys player details.');
  } catch (error) {

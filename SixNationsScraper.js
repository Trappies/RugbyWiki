...
async function scrapeSixNationsFixtures() {
    try {
        const response = await axios.get('https://www.rugbypass.com/six-nations/fixtures-results/');
...
async function updateHTMLWithFixtures(fixtures) {
    try {
        const htmlFilePath = 'SixNations.html';
...
async function scrapeSixNationsStandings() {
    try {
        const response = await axios.get('https://www.rugbypass.com/six-nations/standings/');
        const $ = cheerio.load(response.data);
